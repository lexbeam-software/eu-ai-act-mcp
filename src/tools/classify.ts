...
function classifyFromText(input: ClassifyInput): ClassifyOutput {
  ...
  const prohibitedHit = bestStrongHit<ProhibitedPractice>(combined, prohibitedPractices);
  if (prohibitedHit) {
  ...
  const annexHit = bestStrongHit<HighRiskCategory>(combined, annexIIICategories);
  if (annexHit) {
