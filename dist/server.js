...
export function createServer() {
    ...
    // ── Resources ──
    server.resource("EU AI Act Timeline", "euaiact://timeline", { description: "Key implementation milestones and deadlines of the EU AI Act (Regulation 2024/1689).", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("EU AI Act Risk Levels", "euaiact://risk-levels", { description: "Overview of the four AI Act risk categories: prohibited, high-risk, limited risk, and minimal risk.", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("EU AI Act Annex III — High-Risk Categories", "euaiact://annex/iii", {
    ...
    server.resource("EU AI Act Annex IV — Technical Documentation", "euaiact://annex/iv", {
    ...
    // ── Prompts ──
    server.prompt("classify-my-system", "Classify an AI system under the EU AI Act risk framework. Provide a description and the tool will determine if it's prohibited, high-risk, limited risk, or minimal risk. Provide structured signals (domain, uses_biometrics, etc.) for deterministic classification on canonical cases.", { system_description: z.string().describe("Describe the AI system and its intended use case") }, async ({ system_description }) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
    ...
    server.prompt("compliance-checklist", "Generate a compliance checklist for an AI system based on its risk level and your role (provider or deployer).", {
    ...
    server.prompt("penalty-risk-assessment", "Calculate potential fines for EU AI Act non-compliance based on violation type and company size.", {
    ...
    server.prompt("ground-citation", "Ground a citation to a specific EU AI Act article by retrieving the article text and EUR-Lex URL.", {
