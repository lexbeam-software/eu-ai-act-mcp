import { incidentTriageInputSchema, incidentTriageOutputSchema, } from "../schemas/incident.js";
...
export function registerIncidentTriageTool(server) {
    server.registerTool("euaiact_triage_serious_incident", {
        title: "Triage EU AI Act Serious Incident Reporting",
        description: "Triage whether an AI incident involving a high-risk AI system is reportable under Art. 73 and identify the likely 2-day, 10-day, or 15-day outer reporting bucket. Returns immediate actions, missing facts, clock-start guidance, and role notes for providers/deployers.",
        annotations: {
            readOnlyHint: true,
            idempotentHint: true,
            openWorldHint: false,
        ...
        const missing = [];
        if (input.high_risk_system === undefined)
            missing.push("Confirm whether the incident involves a high-risk AI system.");
        if (input.causal_link_established_or_likely === undefined)
            missing.push("Assess whether a causal link or reasonable likelihood exists between the AI system and the incident.");
        if (!input.aware_date)
            missing.push("Record the date when the provider/deployer became aware of the incident.");
        ...
        let reportable = "insufficient_information";
        let bucket = "insufficient_information";
        let days = null;
        ...
        const output = {
            ...
            clock_start: "Report without undue delay after establishing a causal link or reasonable likelihood. Track the awareness date and the causal-link determination separately.",
            reasoning,
            immediate_actions: [
